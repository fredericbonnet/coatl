...
TEST_SUITE(testNumber, 
    testInt, testFloat
)

TEST_FIXTURE_SETUP(testNumber) {
    Col_Init(COL_SINGLE);
    Col_SetErrorProc(ERROR_PROC);

    Col_PauseGC();
}
TEST_FIXTURE_TEARDOWN(testNumber) {
    if (!TEST_FAIL) {
	Col_ResumeGC();
    }
    Col_Cleanup();
...
#define READ_EXPECTED_INT(expected, value, format) { \
    ...
    ASSERT_MSG(Col_IntWordValue(w) == (expected), "value=%s", value); \
    ASSERT_MSG(Col_RopeIterCompare(begin, end) == 0, "value=%s,begin index=%u", value, Col_RopeIterIndex(begin)); \
}
#define WRITE_EXPECTED_INT(value, expected, format) { \
    ...
    Col_RopeIterator it; \
    \
    ASSERT_MSG(Coatl_WriteIntWord(strbuf, number, format) == (size_t) strlen(expected), "value=%d", value); \
    for (Col_RopeIterFirst(it, Col_StringBufferFreeze(strbuf)); \
	    !Col_RopeIterEnd(it); Col_RopeIterNext(it)) { \
	ASSERT_MSG(Col_RopeIterAt(it) == (Col_Char) expected[Col_RopeIterIndex(it)], "value=%u,index=%u", value, Col_RopeIterIndex(it)); \
    } \
}

TEST_CASE(testIntPredefined, testNumber) {
    int value;
    char buf[100];
...
TEST_CASE(testIntRadix, testNumber) {
    int value;
    char buf[100];
...
TEST_CASE(testIntPrefix, testNumber) {
    int value;
    char buf[100];
    static char *digits  = "0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz";
    static char *digitsL = "0123456789abcdefghijklmnopqrstuvwxyz";
...
#define READ_EXPECTED_FLOAT(expected, value, format, epsilon) { \
    ...
    ASSERT_MSG(Col_FloatWordValue(w) - (expected) <= (epsilon), "value=%s", value); \
    ASSERT_MSG(Col_RopeIterCompare(begin, end) == 0, "value=%s,begin index=%u", value, Col_RopeIterIndex(begin)); \
}
#define WRITE_EXPECTED_FLOAT(value, expected, format, max) { \
    ...
    \
    ASSERT_MSG(len == (size_t) strlen(expected), "value=%g,length=%u", value, len); \
    for (Col_RopeIterFirst(it, Col_StringBufferFreeze(strbuf)); \
	    !Col_RopeIterEnd(it) && Col_RopeIterIndex(it) < max; Col_RopeIterNext(it)) { \
	ASSERT_MSG(Col_RopeIterAt(it) == (Col_Char) expected[Col_RopeIterIndex(it)], "value=%g,index=%u", value, Col_RopeIterIndex(it)); \
    } \
}

TEST_CASE(testFloatPredefined, testNumber) {
    double value;
    char buf[100];
...
TEST_CASE(testFloatPrintf, testNumber) {
